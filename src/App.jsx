...
const C = {
  bg: '#0A2A20',
  bgGrad: 'radial-gradient(1200px 800px at 50% -10%, #123B2C 0%, #0A2A20 55%, #071F18 100%)',
  card: '#0F3226',
  cardAlt: '#123A2C',
...
const DEFAULT_DATA = { players: [], attendance: {}, payments: {}, config: { monthlyFee: 50, adminPin: null } };
...
function buildPixPayload({ amount, description, txid }) {
  ...
  payload += tlv('58', 'BR') + tlv('59', 'FURAO FC'.slice(0, 25)) + tlv('60', 'RIO DE JANEIRO'.slice(0, 15));
  ...
  return (
    <button
      ...
      style={{
        ...
        background: disabled ? 'rgba(245,241,230,0.12)' : danger ? C.danger : C.gold,
        color: disabled ? C.chalkDim : danger ? '#fff' : '#1A1204',
          ...
          {['Vasco', 'Flamengo'].map((t) => (
            <button
              ...
              style={{
                ...
                border: team === t ? `2px solid ${C.gold}` : `1px solid ${C.line}`,
                background: TEAM_STYLE[t].bg, color: TEAM_STYLE[t].text,
  ...
  return (
    <Modal title="Pagar com PIX" onClose={onClose}>
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, color: C.chalkDim, marginBottom: 16 }}>Furão FC · Rio de Janeiro</div>
  ...
  return (
    <div style={{ borderRadius: 20, overflow: 'hidden', border: `1px solid ${C.line}`, background: C.card }}>
      <div style={{ display: 'flex', alignItems: 'stretch', minHeight: 132 }}>
        <div style={{ width: 64, background: TEAM_STYLE.Vasco.bg, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '10px 6px' }}>
          <span style={{ color: TEAM_STYLE.Vasco.text, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 11, textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>VASCO</span>
          <span style={{ color: TEAM_STYLE.Vasco.text, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 13, textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>{fmtBRL(vascoTotal)}</span>
        </div>

        <div style={{ flex: 1, padding: '16px 10px', textAlign: 'center', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div style={{ fontSize: 11, letterSpacing: 1.5, color: C.chalkDim, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, textTransform: 'uppercase' }}>Arrecadado · {monthLabel}</div>
          <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 40, color: C.gold, lineHeight: 1.1, letterSpacing: 1 }}>{fmtBRL(total)}</div>
          <div style={{ height: 6, borderRadius: 999, background: 'rgba(245,241,230,0.10)', overflow: 'hidden', margin: '6px 4px 2px' }}>
            <div style={{ width: `${pct}%`, height: '100%', background: C.gold, transition: 'width .4s' }} />
          </div>
          <div style={{ fontSize: 11, color: C.chalkDim }}>{pct}% da meta do mês ({fmtBRL(possible)})</div>
        </div>

        <div style={{ width: 64, background: TEAM_STYLE.Flamengo.bg, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '10px 6px' }}>
          <span style={{ color: '#fff', fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 11, textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>FLAMENGO</span>
          <span style={{ color: '#fff', fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 13, textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>{fmtBRL(flaTotal)}</span>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ padding: 'calc(18px + env(safe-area-inset-top)) 18px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 26, color: C.chalk, letterSpacing: 1 }}>FURÃO FC</span>
        </div>
...
function BottomNav({ view, setView }) {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(10,42,32,0.94)', backdropFilter: 'blur(6px)',
      ...
      {items.map((it) => {
        ...
        return (
          ...
          >
            <Icon size={20} color={active ? C.gold : C.chalkDim} />
            <span style={{ fontSize: 10, color: active ? C.gold : C.chalkDim, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, letterSpacing: 0.3 }}>{it.label}</span>
          </button>
        );
      })}
    </div>
          ...
          <div style={{ flex: 1, padding: '0 18px 16px', overflowY: 'auto' }}>
            ...
            {view === 'financeiro' && !isAdmin && (
              <TeamFinanceiroView
                ...
                monthOffset={monthOffset} setMonthOffset={setMonthOffset}
                onPay={(amount, mLabel) => setPixModal({ amount, description: `Mensalidade ${mLabel} - ${currentUser.name}`, txid: `FURAOFC${monthKeyFor(new Date()).replace('-', '')}` })}
              />
            )}
            {view === 'perfil' && (
              <PerfilView
                ...
                onShowPix={() => setPixModal({ amount: 0, description: `Contribuição - ${currentUser.name}`, txid: 'FURAOFC' })}
              />
            )}
          </div>
  ...
  return (
    <div style={{ padding: 'calc(28px + env(safe-area-inset-top)) 20px calc(20px + env(safe-area-inset-bottom))', minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ textAlign: 'center', marginBottom: 22 }}>
        ...
        <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 34, color: C.chalk, letterSpacing: 1 }}>FURÃO FC</div>
      ...
      <div style={{ flex: 1 }} />
      <button onClick={onNew} style={{ marginTop: 16, background: 'none', border: `1px solid ${C.line}`, borderRadius: 14, padding: '13px 16px', color: C.chalk, fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>
...
function InicioView({ data, monthKey, monthLabel, nextMatch, attendanceArr, confirmedVasco, confirmedFla, currentUser, toggleAttendance, setView }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 16, background: C.card, border: `1px solid ${C.line}`, borderRadius: 18, padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div>
            ...
            <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 18, color: C.chalk }}>{dateLabel} · 19h</div>
          </div>
      ...
      {[['Vasco', vasco], ['Flamengo', fla]].map(([team, list]) => (
        <div key={team} style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
            <div style={{ width: 8, height: 8, borderRadius: 999, background: TEAM_STYLE[team].dot }} />
  ...
  return (
    <div>
      <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 18, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 19, color: C.chalk, marginBottom: 12 }}>{dateLabel} · 19h</div>
        <PrimaryButton onClick={() => toggleAttendance(currentUser.id)} danger={iConfirmed}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        <div style={{ width: 8, height: 8, borderRadius: 999, background: ts.dot }} />
...
function PerfilView({ currentUser, isAdmin, onLogout, onEdit, onSettings, onShowPix }) {
  ...
  return (
    <div>
      <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 18, padding: 20, textAlign: 'center', marginBottom: 16 }}>
